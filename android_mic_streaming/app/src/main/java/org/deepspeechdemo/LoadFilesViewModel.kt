...
internal class LoadFilesViewModelFile {
    ...
    val path: LiveData<String> = Transformations.map(uri) { it?.toString() }
}
...
internal class LoadFilesViewModel: ViewModel() {
    ...
    val nextIsDone = MediatorLiveData<Boolean>().apply {
        addSource(model.uri) { value = it != null && scorer.uri.value != null }
        addSource(scorer.uri) { value = it != null && model.uri.value != null }
    }
}
