...
internal fun Class<out Activity>.start(context: Context) {
    if (context is Activity) {
        doStart(context)
    } else {
...
internal fun Uri.getFd(contentResolver: ContentResolver) =
    try {
...
internal fun ParcelFileDescriptor.getFile() =
    File("/proc/self/fd/$fd").takeIf { it.exists() }
