        ...
        DataBindingUtil.setContentView<ActivityLoadFilesBinding>(
            ...
            vm = ViewModelProvider(this@LoadFilesActivity).get<LoadFilesViewModel>().apply {
                fun TwoLineItemBinding.bind(item: LoadFilesViewModelFile) {
                    ...
                    }) { contentResolver.run { item.uri.value = it } }
                    root.setOnClickListener { launcher.launch(arrayOf("*/*")) }
                ...
                nextButton.setOnClickListener { when {
                    nextIsDone.value == true -> { contentResolver.run {
                        val uris = mutableListOf<Uri>()
                        getDefaultSharedPreferences(this@LoadFilesActivity).edit().run {
                            fun save(key: String, item: LoadFilesViewModelFile) {
                                item.uri.value!!.let {
                                    uris.add(it)
                                    takePersistableUriPermission(it, FLAG_GRANT_READ_URI_PERMISSION)
                                    putString(key, "$it")
                                }
                            }
                        ...
                        persistedUriPermissions.forEach {
                            ...
                            if (!uris.contains(uri)) {
                                releasePersistableUriPermission(uri, FLAG_GRANT_READ_URI_PERMISSION)
                            }
                        }
                        (application as DSApp).reloadModel()
                        finish()
                    } }
                    model.path.value == null -> { modelFileListItem.root.triggerRipple() }
                    scorer.path.value == null -> { scorerFileListItem.root.triggerRipple() }
